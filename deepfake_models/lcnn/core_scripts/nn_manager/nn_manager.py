...
#     """
#     nii_display.f_print_w_date("Start inference (generation)")
#     nii_display.f_eprint("Start inference (generation)")
...
#     print("Generate minibatch blahblah indexed within [{:d},{:d})".format(
...
#         nii_display.f_print("Test data loader: ", str(type(test_data_loader)))
#         # run generation
#         for data_idx, (data_in, data_tar, data_info, idx_orig) in \
#             enumerate(test_data_loader):
            
#             nii_display.f_print("Data info type: ", str(type(data_info))
#             # nii_display.f_print(data_info.shape)
#             # nii_display.f_print(str(data_info[0]))
...
#                 print("Now calling pt_model.inference")
#                 nii_display.f_print("Now calling pt_model.inference")
...
#                     sys.exit(1)
#                 nii_display.f_print("Printing data_gen")
...
#                                     'warning')
#                 nii_display.f_print("test test")
#                 nii_display.f_print("Data info: ", str(data_info))
#                 nii_display.f_print("Data gen: ", str(data_gen))
#                 nii_display.f_print(data_gen.shape)
#                 return data_gen
#             else:
...
def f_inference_wrapper(args, pt_model, device, 
    ...
    ### using the loaded model to perform inference on the test data
    # load trained model parameters from checkpoint
    nii_nn_tools.f_load_checkpoint_for_inference(checkpoint, pt_model)
    ...
    # start generation
    pt_model.eval()             ### sets model to eval mode
    total_start_time = time.time()
    total_accumulate = 0
    sample_account = 0
    with torch.no_grad():
